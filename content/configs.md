...
Cloud config must be specified for the Director to successfully complete a deploy. Other configs—such as runtime, CPI, and deploy—are optional and may be set as needed. Given that functionality of saving, retrieving, viewing, diffing, and listing for different configs is very similar, Director provides a consolidated CLI and API functionality to support all these actions.
