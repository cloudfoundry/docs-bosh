...
- **<value>** [String]: Password value. A random string containing a fixed set of characters: lowercase letters (from `a` to `z`) and figures (from `0` to `9`).
...
- **private_key** [String]: Private key (PEM encoded).
    Since [Sept 8th, 2017][boshcli_priv_key_len], the Bosh CLI generates private keys which are `3072` bits long, and doesn't provide any parameter for this, whereas CredHub default [is 2048][credhub_gen_cert_opts].
