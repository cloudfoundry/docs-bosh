...
* **use\_tmpfs\_job\_config** [Boolean, optional]: Mounts all directories which contain [rendered job templates on tmpfs](creds-tmpfs.md) by default on every instance group. Default `false`.
...
* **update_mode** [String, optional]: Update mode to use when generating credentials. Currently supported update modes are `no-overwrite`, `overwrite`, and `converge`. Defaults to `no-overwrite`. For further information, please refer to the [CredHub documentation](https://credhub-api.cfapps.io/version/2.5/#_overwriting_credential_values).
